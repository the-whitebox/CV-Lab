...
Future<bool> callFavoriteApi(int index, String token) async {
  ...
  final Map<String, dynamic> payload = {'template_id': 'v10${index + 1}'};
  ...
  try {
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      customLog('API call successful: ${response.body}');
      return true; // CV added successfully
    } else if (response.statusCode == 422) {
      customLog('CV is already in favourites');
      return false; // CV is already in favourites
    } else {
      customLog('Failed API call. Status code: ${response.statusCode} data: $payload');
      customLog('Response: ${response.body}');
      return false; // Failed to add CV
    }
  } catch (e) {
    customLog('Error making API call: $e');
...
Future<List<String>> fetchFavoriteCVs(String token) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      customLog('Failed to fetch favorite CVs. Status code: ${response.statusCode}');
      return []; // Return an empty list or handle the error as needed
    }
  } catch (e) {
    customLog('Error fetching favorite CVs: $e');
    return []; // Return an empty list or handle the error as needed
  }
}
...
Future<void> removeFromFavorites(int templateId, String token) async {
  ...
  customLog('template id:::::: $templateId');
  try {
    ...
    if (response.statusCode == 200) {
      customLog('CV removed from favorites successfully');
    } else {
      customLog('Failed to remove CV from favorites. Status code: ${response.statusCode}');
    }
  } catch (e) {
    customLog('Error removing CV from favorites: $e');
  }
}
  ...
  try {
    ...
    if (response.statusCode == 200) {
      ...
      if (responseData != null) {
        ...
        customLog('Error: Response data is null');
        return [];
      }
    } else {
      customLog('Failed to fetch My CVs data. Status code: ${response.statusCode}');
      customLog('Response: ${response.body}');
      return [];
    }
  } catch (e) {
    customLog('Error fetching My CVs data: $e');
    return [];
  }
}

Future<void> removeFromMyCVs(int cvId, String templateId, String token) async {
  const String apiEndpoint = '$baseUrl/api/deleteCV/';
  try {
    final response = await http.delete(
      Uri.parse(apiEndpoint),
      headers: {
    ...
    if (response.statusCode == 200) {
      customLog('CV removed from My CVs successfully');
    } else {
      customLog('Failed to remove CV from My CVs. Status code: ${response.statusCode}');
    }
  } catch (e) {
    customLog('Error removing CV from My CVs: $e');
  }
}
...
Future<void> updateCV(int cvId, String templateId, String token) async {
  ...
  try {
    ...
    if (response.statusCode == 200) {
      customLog('response :::: ${response.body}');
      customLog('CV updated successfully');
    } else {
                    ...
                    child: Column(children: [
                      Container(
                          ...
                          child: TabBar(
                              ...
                              tabs: [
                                Tab(child: Text('Templates', style: TextStyle(fontSize: 20, color: _tabController.index == 0 ? kHighlightedColor : Colors.black))),
                                Tab(child: Text('My CV\'s', style: TextStyle(fontSize: 20, color: _tabController.index == 1 ? kHighlightedColor : Colors.black))),
                                Tab(child: Text('Favourites', style: TextStyle(fontSize: 20, color: _tabController.index == 2 ? kHighlightedColor : Colors.black)))
                              ])),
                      Expanded(child: TabBarView(controller: _tabController, children: [buildTemplateTab(), buildMyCVsTab(), buildFavouritesTab()]))
                    ])))));
  }
        ...
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            ...
            return GridView.builder(
                gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(childAspectRatio: 0.70, crossAxisCount: 2, crossAxisSpacing: 10, mainAxisSpacing: 10),
          ...
          child: Container(
              ...
              child: TextField(
                  ...
                  decoration: InputDecoration(
                      ...
                      focusedBorder: const OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(10.0)), borderSide: BorderSide(color: Colors.transparent)),
                      enabledBorder: const OutlineInputBorder(borderRadius: BorderRadius.all(Radius.circular(10.0)), borderSide: BorderSide(color: Colors.transparent)),
              ...
              builder: (context, snapshot) {
                if (snapshot.hasError) {
                  ...
                  return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                    Image.asset(AppImages.sleepyDog),
                    const SizedBox(height: 10.0),
                    Text('No Collection', style: kFont24.copyWith(color: kHighlightedColor)),
                    Text('You have not saved any CV yet', style: kFont12.copyWith(color: const Color(0xFF4E4949)))
                  ]);
                }
              }))
    ]);
    ...
    return Column(children: [
      Stack(children: [
        Container(
            ...
            decoration: const BoxDecoration(boxShadow: [BoxShadow(color: Colors.black12, blurRadius: 0.545, offset: Offset(-5, 2), spreadRadius: -3)]),
            child: GestureDetector(
                ...
                child: Stack(children: [
                  Column(children: [Image.asset(pdfImages[indexOfMyCV]), const SizedBox(height: 3)]),
                    ...
                    Center(
                      ...
                      SizedBox(
                          ...
                          child: ElevatedButton(
                              ...
                              child: const Text('Update', style: kFont20White)))
                    ]))
                ]))),
        Align(
            ...
            child: IconButton(
                ...
                icon: Image.asset(AppImages.cancelCV, height: 25, width: 25)))
      ]),
      Row(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [const SizedBox(width: 5), const Text('Title: '), Expanded(child: Text(title, style: const TextStyle(fontWeight: FontWeight.w600)))])
    ]);
  }
        ...
        builder: (context, snapshot) {
          if (snapshot.hasError) {
            ...
            return Column(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              const SizedBox(height: 10.0),
              Text('No Collection', style: kFont24.copyWith(color: kHighlightedColor)),
              Text('Go to templates to add favorite CV', style: kFont12.copyWith(color: const Color(0xFF4E4949)))
            ]);
          }
        });
  }
                  ...
                  if (tappedFavCVIndex == index)
                    Center(
                        child: ElevatedButton(
                            ...
                            child: const Text('Select', style: kFont20White))),
                  Align(
                      ...
                      child: IconButton(
                          ...
                          icon: Image.asset('assets/images/cancelCV.png', height: 25, width: 25)))
                ]))));
  }
        ...
        child: Container(
            ...
            child: Stack(children: [
              ...
              if (tappedIndex == index)
                Center(
                    child:
                        ElevatedButton(style: kElevatedButtonPrimaryBG, onPressed: () => Get.toNamed(pdfFiles[index]), child: const Text('Select', style: kFont20White))),
              Align(
                  alignment: const Alignment(1.25, -1.2),
                  child: IconButton(
                      icon: Image.asset(isFavouriteItem ? 'assets/images/favourite-red.png' : 'assets/images/favouriteCV.png', height: 25, width: 25),
                      onPressed: () async {
                        if (!isFavouriteItem) {
